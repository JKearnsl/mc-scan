...
use crate::components::{
    action_button,
    ui::{BtnVariant, btn, divider},
};
use crate::styles::{MONO, SANS, SANS_SEMIBOLD, c, is_dark};
...
pub fn render(app: &McScan) -> Element<'_, Message> {
    ...
    let top_row = row![
        ...
        btn(
            BtnVariant::Icon {
                handle: settings_icon,
                size: 20.0
            },
            Message::OpenModal(ModalKind::Settings)
        ),
    ]
    ...
    let total_str = if total_hosts == u64::MAX {
        "∞".to_string()
    } else {
    ...
    let ranges_header = row![
        text(format!("{} · {}", tr.ip_ranges, range_count))
            .size(11)
            .font(SANS_SEMIBOLD)
        ...
        btn(
            ...
            Message::OpenModal(ModalKind::AddRanges)
        ),
    ]
    .align_y(Alignment::Center);

    let total_row = row![
        text(tr.total_addresses)
            .size(12)
            .font(SANS)
            .style(|t: &Theme| text::Style {
                color: Some(if is_dark(t) {
